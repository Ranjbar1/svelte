...
export function useTranscriber(): Transcriber {
	...
	let model = $state<string>(Constants.DEFAULT_MODEL);
	let subtask = $state<string>(Constants.DEFAULT_SUBTASK);
	let quantized = $state<boolean>(Constants.DEFAULT_QUANTIZED);
	let multilingual = $state<boolean>(Constants.DEFAULT_MULTILINGUAL);
	let language = $state<string>(Constants.DEFAULT_LANGUAGE);

	const onInputChange = () => (transcript = undefined);
